...
const createDerivedControl: {
  /**
   * Creates a readonly {@link ControlScope control} mirroring the given async
  ...
  /** Creates a readonly {@link ControlScope control} mirroring the given {@link control}'s value. */
  <T>(control: ReadonlyControl<T>): ControlScope<T>;
  /**
   * Creates a readonly {@link ControlScope control} holding the async
   * {@link control}'s value passed through {@link mapper}. The {@link mapper}
   * runs on every change and must handle `undefined` (the not-loaded state).
   *
   * @example
   * ```ts
  ...
  /**
   * Creates a readonly {@link ControlScope control} holding the
   * {@link control}'s value passed through {@link mapper}.
   */
  <T, V>(control: ReadonlyControl<T>, mapper: (value: T) => V): ControlScope<V>;
  /**
   * Creates a readonly {@link ControlScope control} combining multiple
