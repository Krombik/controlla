...
import type { PollableMethods, AsyncRootNode } from '#internal/types';
...
export class PollingProcess implements PollableMethods {
...
function commonSleep(this: PollingProcess) {
  const self = this;
...
function smartSleep(this: PollingProcess) {
  const self = this;
...
export const polling: Parameters<
  typeof createLoadRunner<PollingProcess>
>[0] = async (load, cancelPromise, self) => {
  const loadingProcess: PollingProcess = self._loadProcess;
